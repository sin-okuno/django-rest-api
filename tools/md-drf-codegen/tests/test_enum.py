...
def test_enum_array_uses_comma_separated_list_field() -> None:
    spec = ApiSpec(
        version=1,
        apis=[],
        constants=[
            ConstantDefinition(path="common/util/Consts.py", className="Status"),
        ],
        types={
    ...
    files = generate_code_files(spec, target=GenerateTarget.SERIALIZER, package_name="sample")
    content = files["sample_serializers.py"]
    assert "class CommaSeparatedListField(serializers.ListField):" in content
    assert "CommaSeparatedListField(child=serializers.ChoiceField(" in content
    assert "CommaSeparatedListField(child=serializers.CharField(" in content
    assert _CHOICES_STATUS in content

    doc = build_openapi_document(spec, title="Sample")
    # Direct schema check via a fake GET endpoint would be heavier; verify field schema path
    # through components after adding a wrapper type used as query is enough via statuses property.
    statuses = doc["components"]["schemas"]["ProductListRequest"]["properties"]["statuses"]
    assert statuses["type"] == "array"
    assert statuses["items"]["enum"] == [1, 2, 3]
